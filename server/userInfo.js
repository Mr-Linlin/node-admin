...
const loginInfo = (req, res) => {
    ...
    const sql = 'select *from tm_ocp_user where accountnumber=?'
    db.query(sql, accountnumber, (err, results) => {
        ...
        // 判断用户状态是否处于可用
        if (results[0].userstatus !== 1 && results[0].userstatus !== '1') {
        ...
        if (results[0].userpower !== 1 && results[0].userpower !== '1') {
...
const userList = (req, res) => {
    ...
    let sql = 'select * from tm_ocp_user'
    db.query(sql, (err, results) => {
        if (err) return res.send({ code: 201, msg: err.message })
        let total = results.length
        let sql = `select * from tm_ocp_user limit ${offset},${limit}`
        db.query(sql, (err, results) => {
            if (err) return res.send({ code: 201, msg: err.message })
